...
const get_connection_error_label = (retry_delay_secs: number): string => {
    if (original_retry_delay_secs < 5) {
        ...
        return $t({defaultMessage: "Unable to connect to JSlack. Trying to reconnect soon…"});
    }
    return $t(
        {
            defaultMessage:
                "Unable to connect to JSlack. {retry_delay_secs, plural, one {Trying to reconnect in {retry_delay_secs} second…} other {Trying to reconnect in {retry_delay_secs} seconds…}}",
        },
        {retry_delay_secs},
    );
};
...
function retry_connection_click_handler(e: JQuery.ClickEvent, on_retry_callback: () => void): void {
    ...
    $banner
        .find(".banner-label")
        .text($t({defaultMessage: "Unable to connect to JSlack. Trying to reconnect soon…"}));
